 ...
 * Usage: First create a
 * <code>STLFileReader</code> object. To obtain the number of objects, name of
 * objects and number of facets for each object use the appropriate methods.
 * Then use the {@link #getNextFacet} method repetitively to obtain the
 * geometric data for each facet. Call {@link #close} to free the resources.<p>
 * In case that the file uses the binary STL format, no check can be done to
 * assure that the file is in STL format. A wrong format will only be recognized
 * if an invalid amount of data is contained in the file.<p>
 *
 * @author Dipl. Ing. Paul Szawlowski - University of Vienna, Dept. of Medical
 * Computer Sciences
...
public class STLFileReader {
    ...
    /**
     * Creates a
     * <code>STLFileReader</code> object to read a STL file from a file. The
     * data may be in ASCII or binary format.
     *
    ...
    /**
     * Creates a
     * <code>STLFileReader</code> object to read a STL file from an URL. The
     * data may be in ASCII or binary format.
     *
     ...
     * Creates a
     * <code>STLFileReader</code> object to read a STL file from an URL. The
     * data may be in ASCII or binary format. A progress monitor will show the
    ...
    /**
     * Returns the data for a facet. The orientation of the facets (which way is
     * out and which way is in) is specified redundantly. First, the direction
     * of the normal is outward. Second, the vertices are listed in
     ...
     * <code>normal</code> and <code>vertices</code>. <code>False</code> if end
     * of file is reached. Further calls of this method after the end of file is
     * reached will lead to an IOException.
     */
     ...
     *
     * @return Array of strings with names of objects. Size of array = number of
     * objects in file. If name is not contained then the appropriate string
     * is <code>null</code>.
     */
