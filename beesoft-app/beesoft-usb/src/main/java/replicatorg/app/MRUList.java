...
/**
 * The Most-Recently-Used list of file paths. Every time a file is explicitly
 * opened by the user, it should be placed at the front of the MRU list. If the
 * file previously appeared in the list, the old entry is removed; otherwise the
 * list grows. When the list size is exceeded, the last entry is removed. The
 * list is rewritten to the preferences on every update.
 *
 * Due to limitations in the length of parameter values, the number of paths
 * stored may be smaller than the maximum number of entries.
 *
 * @author phooky
 *
 */
public class MRUList implements Iterable<String> {
    ...
    private MRUList() {
        String mruString = ProperDefault.get(MRU_LIST_KEY);
    ...
    private void writeToPreferences() {
        ...
        ProperDefault.put(MRU_LIST_KEY, sb.toString());
    }
