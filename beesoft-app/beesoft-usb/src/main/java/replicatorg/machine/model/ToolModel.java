...
public class ToolModel {
    ...
    private boolean extruderTemperatureAvailable = false;
    private boolean blockTemperatureAvailable = false;
    ...
    /**
     *
     * @return null if motorstepperaxis wasn't specified. The axis identifier
     * otherwise
     */
    ...
    public void setCurrentTemperature(int extruderTemperature, int blockTemperature) {
        synchronized (this.extruderTemperature) {
            ...
            this.extruderTemperature.notifyAll();
        }

        synchronized (this.blockTemperature) {
            this.blockTemperature.set(blockTemperature);
            blockTemperatureAvailable = true;
            this.blockTemperature.notifyAll();
        }
    }
    ...
    public int getExtruderTemperature() {
        synchronized (extruderTemperature) {
            if (extruderTemperatureAvailable == false) {
                try {
                    extruderTemperature.wait(3000);
                } catch (InterruptedException ex) {
                }
            }

            extruderTemperatureAvailable = false;
    ...
    public int getBlockTemperature() {
        synchronized (blockTemperature) {
            if (blockTemperatureAvailable == false) {
                try {
                    blockTemperature.wait(3000);
                } catch (InterruptedException ex) {
                }
            }

            blockTemperatureAvailable = false;
