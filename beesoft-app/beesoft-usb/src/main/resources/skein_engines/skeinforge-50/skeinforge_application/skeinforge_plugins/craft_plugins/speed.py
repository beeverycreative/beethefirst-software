...
class SpeedRepository:
	...
	def __init__(self):
		...
		self.fanSwitch = settings.BooleanSetting().getFromValue('Turn Fan Off at First Layer', self, True)
...
class SpeedSkein:
	...
	def addFlowRateLine(self):
		...
		if self.layerIndex == 0:
			if self.repository.fanSwitch.value==True:
				self.distanceFeedRate.addLine('M107 ; First Layer Blower OFF')
				self.repository.fanSwitch.value=False
			if self.isEdgePath:
		...
		if self.layerIndex > 0:
			if self.repository.fanSwitch.value==False:
				self.distanceFeedRate.addLine('M106 ; First Layer Blower ON')
				self.repository.fanSwitch.value=True
